...
from model_shufflenetv2 import *
import random
import numpy as np
import datetime
...
fh = logging.FileHandler(os.path.join(log_path, 'train-{}{:02}{}-shufflenetv2warmup-RMSprop-lambda'.format(local_time.tm_year % 2000, local_time.tm_mon, t)))
...
net = ShuffleNetV2(num_classes=3)
net.to(device)
loss_function = nn.CrossEntropyLoss()
#optimizer = optim.Adam(net.parameters(), lr=0.0003)S
...
for epoch in range(300):
    ...
    for step, data in enumerate(train_loader, start=0):
        ...
        printInfo = 'TRAIN Iter {}: lr = {:.6f},\tloss = {:.6f},\t'.format(epoch*len(train_loader) + step, scheduler_warmup.get_lr()[0],loss.item())
    ...
    with torch.no_grad():
        ...
        filename = os.path.join(os.path.join(compare_result, "ShufflenetV2-{:06}warmup-RMSprop-lambda.pth.tar".format(epoch+1)))
        ...
        with open(
                '/media/jie/DATA/Junjie/PhD_project/Classification-Task/ShuffleNet-Series-master/DetNAS_models/ShuffleNetV2/log-RMSprop-lambda.txt',
                'a+') as fd:
