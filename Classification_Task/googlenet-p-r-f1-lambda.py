...
fh = logging.FileHandler(os.path.join(log_path, 'train-{}{:02}{}-googlenetwarmup-SGD-lambda'.format(local_time.tm_year % 2000, local_time.tm_mon, t)))
...
for epoch in range(300):
    ...
    with torch.no_grad():
        ...
        filename = os.path.join(os.path.join(compare_result, "googlenet-{:06}warmup- SGD-lambda.pth.tar".format(epoch+1)))
        ...
        with open(
                '/media/jie/DATA/Junjie/PhD_project/Classification-Task/ShuffleNet-Series-master/DetNAS_models/GoogLeNet/log- SGD-lambda.txt',
                'a+') as fd:
